...
import { Bell, Search, CheckCircle2, Info, Sun, Moon, X, LogOut, Menu, Shield,
...
export function Header() {
  ...
  const ACCOUNTANT_TABS = ['Profile', 'BIR Configurations', 'Categories', 'Security', 'Notifications'];
  const EXEC_TABS = ['General', 'Organization', 'Access & Roles', 'Security', 'Data & Privacy'];
    ...
    <>
      ...
      <div 
        className={cn(
          ...
          (showMobileMenu && location.pathname.startsWith('/settings')) ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
        )}
      >
        ...
        <div
          ref={drawerRef}
          className={cn(
            "relative w-72 h-full flex flex-col shadow-2xl transition-transform duration-300 ease-out transform",
            (showMobileMenu && location.pathname.startsWith('/settings')) ? "translate-x-0" : "-translate-x-full"
          )}
        ...
        >
          <div className="h-14 flex items-center justify-between px-4" style={{ boxShadow: '0 1px 0 var(--border-subtle)' }}>
            ...
            <button
              ...
              className="p-1.5 rounded-lg text-[--text-muted] hover:text-[--text-primary] hover:bg-[--bg-raised] transition-all duration-200"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto py-4 px-3 space-y-0.5">
            ...
            {settingsTabs.map((tab) => {
              ...
              const tabIcons: Record<string, React.ReactNode> = {
                ...
                'Security': <Shield className="h-5 w-5 shrink-0" />,
              ...
              return (
                <button
                  ...
                  className={cn(
                    'flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 text-left active:scale-[0.98]',
                    isActive
                      ? 'bg-blue-50 dark:bg-[#1e2d45] text-blue-700 dark:text-blue-400'
                      : 'text-[--text-secondary] hover:bg-[--bg-raised] hover:text-[--text-primary]'
                  )}
                >
                  <span className={cn('shrink-0 transition-transform duration-200', isActive ? 'text-blue-600 dark:text-blue-400 scale-110' : 'text-[--text-muted]')}>
                    {tabIcons[tab]}
                  </span>
                  <span>{tab}</span>
                  {isActive && (
                    <svg className="ml-auto h-4 w-4 text-blue-600 dark:text-blue-400 animate-page-in" fill="none" stroke="currentColor" viewBox="0 0 24 24">

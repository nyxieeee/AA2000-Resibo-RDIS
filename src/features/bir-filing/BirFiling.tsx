    ...
    <div className="max-w-6xl mx-auto space-y-4 md:space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        <div className="md:col-span-1 space-y-4 md:space-y-6">
          ...
          <div className="bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-900 rounded-xl p-4 md:p-5 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-bold text-green-900 dark:text-green-400">Workspace Settings</h3>
              <span className="text-xs font-bold bg-green-200 dark:bg-green-900 text-green-800 dark:text-green-400 px-2 py-0.5 rounded-full">READY</span>
            </div>
        ...
        <div className="md:col-span-2 bg-[--bg-surface] border border-[--border-subtle] rounded-xl p-4 md:p-6 shadow-sm flex flex-col">
          ...
          <div className="flex-1 border border-[--border-subtle] rounded-lg overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left" style={{ minWidth: '400px' }}>
                <thead className="bg-[--bg-raised] font-medium text-[--text-muted] border-b border-[--border-subtle]">
                  <tr>
                  ...
                  {stats.warnings.length === 0 ? (
                    ...
                    <tr key={doc.id} className="bg-rose-50/30 dark:bg-rose-950/10">
                      <td className="px-4 py-3">
                        <span className="font-medium text-[--text-primary]">{doc.vendor}</span>
                      ...
                      <td className="px-4 py-3">
                        ...
                        {doc.taxType && doc.taxType !== 'VAT' && (
                          <div className="text-right mt-0.5">
                            <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-purple-500/20 text-purple-400">{doc.taxType}</span>
                          </div>
                        )}
                      </td>
                      <td className="px-4 py-3 text-right text-[--text-primary]">₱ {doc.vat.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                      <td className="px-4 py-3 text-right text-[--text-primary]">₱ {doc.zeroRatedSales.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

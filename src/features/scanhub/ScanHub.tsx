...
export function ScanHub() {
  ...
  const handleCameraCapture = async (blob: Blob) => {
    ...
    try {
      ...
      const extracted = await callGroqVision(apiKey, base64, mediaType);
      setProcessingStep('Building document record...');
      const doc = buildDocumentRecord(null, extracted, base64, mediaType, `camera-${Date.now()}.jpg`);
      addDocument(doc);
      navigate(`/documents/${doc.id}`);
    } catch (err: unknown) {
  ...
  return (
    <>
      ...
      <div className="max-w-4xl mx-auto space-y-5">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-slate-900 dark:text-white">ScanHub</h1>
        ...
        <div className="grid grid-cols-2 gap-3 md:gap-4">
          <button
            ...
            className={`flex flex-col items-center justify-center gap-3 p-5 md:p-8 rounded-2xl border transition-all text-center
              ${isProcessing || !apiKey
                ? 'border-slate-200/70 dark:border-slate-700/50 opacity-50 cursor-not-allowed'
                : 'border-slate-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-700 hover:bg-blue-50/50 dark:hover:bg-blue-950/20 cursor-pointer active:scale-95'
              }`}
          ...
          >
            ...
            <div>
              <p className="font-semibold text-sm md:text-base text-slate-800 dark:text-white">Take Photo</p>
              <p className="text-xs text-slate-400 mt-0.5 hidden md:block">Use your camera to photograph a receipt</p>
            </div>
          </button>
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center gap-3 p-5 md:p-8 rounded-2xl border border-dashed transition-all text-center
              ${isProcessing || !apiKey
                ? 'border-slate-200/70 dark:border-slate-600/50 opacity-50 cursor-not-allowed'
                : isDragging
                  ? 'border-blue-400 bg-blue-500/10'
                  : 'border-slate-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-700 hover:bg-blue-500/5 cursor-pointer active:scale-95'
              }`}
          >
            ...
            <div>
              <p className="font-semibold text-sm md:text-base text-slate-800 dark:text-white">Upload File</p>
              <p className="text-xs text-slate-400 mt-0.5 hidden md:block">JPG, PNG, WEBP, or GIF (max 10MB)</p>
            </div>
          </button>
        </div>
        ...
        {isProcessing && (
          <div className="flex flex-col items-center gap-3 py-10 rounded-2xl" style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border-default)' }}>
            ...
            <h3 className="text-base font-semibold text-slate-900 dark:text-white">Analyzing with Llama 4 Scout...</h3>

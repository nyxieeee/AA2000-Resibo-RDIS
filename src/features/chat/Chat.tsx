...
function buildSystemPrompt(documents: DocumentRecord[]): string {
  const docSummary = documents.map(d =>
    `- ID:${d.id} | ${d.vendor} | TIN:${d.taxId} | ${d.type} | ${d.docNum} | ${d.date} | ₱${d.total.toLocaleString()} | VAT:₱${d.vat?.toFixed(2)} | Vatable:₱${d.vatableSales?.toFixed(2)} | TaxType:${d.taxType || 'VAT'} | Category:${d.category} | Status:${d.status} | Confidence:${d.confidence}%`
  ).join('\n');
...
export function Chat() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      sender: 'ai',
      text: `Hello! I am your AA2000 RDIS Assistant powered by Llama 3.3 70B via Groq. You currently have ${documents.length} processed documents in your ledger. Ask me anything — totals, vendor queries, VAT breakdowns, or BIR filing figures.`,
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    setInput('');
    setError(null);
    // Re-focus the input immediately after clearing it
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700/50 shrink-0 bg-[--bg-surface]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[--text-muted]">Query your financial ledger using natural language • {documents.length} docs loaded</p>
          </div>
        </div>
      </div>
